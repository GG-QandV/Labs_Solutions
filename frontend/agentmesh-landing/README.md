...
- Traefik: `/srv/traefik/dynamic/acp-gateway.yml` — роутер `aspgateway` с `priority: 100` ведёт только `/agents/*` и `/api/v1/agentmesh/*` на gatewayd `172.18.0.1:8348`; остальное на домене обслуживает nginx-фронт. `noindex` оставлен только на API-роутере, лендинг индексируется.

# ICONS — замена плейсхолдеров на SVG из локальной коллекции
...
Вставить inline-SVG **вместо** `<span>`, сохранив класс и tone:

```html
<svg class="ico ico--lg" data-tone="accent" viewBox="0 0 24 24" fill="none"
...
После замены удалить у `.ico` правило `::after` не нужно — оно применяется только к `<span>` без содержимого;
у `<svg class="ico">` следует убрать `border` через дополнительный класс:
...
(добавить в конец `style.css` одним блоком, когда все иконки заменены).

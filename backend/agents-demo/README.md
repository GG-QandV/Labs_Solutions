...
| Что | Зачем |
|---|---|
| **Cache Rule: Bypass cache** на `/api/*` | SSE нельзя кэшировать; закэшированный поток = замерший экран у второго зрителя |
| **Rocket Loader: Off** (для этого хоста) | он откладывает и переписывает инлайн-JS, ломая инициализацию `EventSource` |
| **Auto Minify / Brotli** — проверить на `/api/*` | сжатие потоковых ответов добавляет буферизацию |
...
Если исключения настраивать не хочется — альтернатива: **DNS-only** (серое облако) для этого
поддомена. TLS всё равно даёт Traefik + Let's Encrypt, но теряются WAF и защита от DDoS,
а IP origin становится виден. Для публичного демо предпочтителен проксированный вариант
с исключениями выше.

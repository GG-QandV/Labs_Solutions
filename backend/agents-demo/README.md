...
Ответ отдаётся с `Cache-Control: no-store` и `X-Accel-Buffering: no`. Без второго заголовка
nginx/прокси буферизует поток и пошаговость исчезает — событий не видно до самого конца.
Для Cloudflare нужен Cache Rule `bypass` на `/api/*` (в SKELETON помечено как непроверенное —
проверить при первом деплое).
...
- Поведение SSE через Cloudflare с активным Cache Rule на практике не проверялось.
